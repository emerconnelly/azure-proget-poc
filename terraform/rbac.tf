...
resource "azurerm_role_assignment" "aks_csi_driver_identity" {
  scope              = azurerm_key_vault.this.id
  role_definition_id = data.azurerm_role_definition.key_vault_administrator.id
  principal_id       = azurerm_kubernetes_cluster.this.key_vault_secrets_provider[0].secret_identity[0].object_id
}

resource "azurerm_role_assignment" "aks_application_gateway_ingress_controller" {
...
# resource "azurerm_user_assigned_identity" "aks_csi_driver_identity" {
...
#   scope              = azurerm_key_vault.this.id
#   role_definition_id = data.azurerm_role_definition.key_vault_administrator.id
#   principal_id       = azurerm_user_assigned_identity.aks_csi_driver_identity.principal_id
# }
...
#   name                = azurerm_resource_group.this.name
#   resource_group_name = azurerm_resource_group.this.name
#   parent_id = azurerm_user_assigned_identity.aks_csi_driver_identity.id # replace(azurerm_kubernetes_cluster.this.key_vault_secrets_provider[0].secret_identity[0].user_assigned_identity_id, "resourcegroups", "resourceGroups")
